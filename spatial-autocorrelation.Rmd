...
Spatial autocorrelation measures the degree of correlation to itself through space (Cliff and Ord 1973, 1981). Tests of spatial autocorrelation examine the independency of observed value of a variable in relation to values of that variable at neighboring locations. Spatial autocorrelation can be positive or negative.
...
```{r}
df[6] <- lapply(df[6], as.numeric) # Cancer data to numeric
...
Before autocorrelation, analysis we have construct neighbours list from polygon list and then we have calculate spatial weights for neighbours lists. 
...
```{r}
moran.test(SPDF$cancer,neighbourhood_weights_list)
```
...
```{r}
gobal.moran.mc <- moran.mc(SPDF$cancer,
...
```{r}
local.moran.results <- localmoran(SPDF$cancer,
                         neighbourhood_weights_list,
...
```{r}
geary.test(SPDF$cancer,neighbourhood_weights_list)
```


```{r}
gobal.geary.mc <- geary.mc(SPDF$cancer,
                                neighbourhood_weights_list,
                                nsim=599)
