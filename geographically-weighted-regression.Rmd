...
knitr::include_graphics('F:\\Spatial_Data_Processing_and_Analysis_R\\Spatial-data-R\\Image\\PNG_FILE_07\\GWR_OLS.png')
```
...
In R, two packages are generally recommend for GWR modeling:  


* [GWR with spgwr package](#gwr-with-spgwr-package)

* [GWR with GWModel package](#gwr-with-gwmodel-package)
...
In this exercise we will apply a general form GWR  predict Soil Organic C.  We will use two covariates, elevation and NDVI to fit GWR model. The soil organic carbon data (train and test data set) could be found [here](https://www.dropbox.com/s/d6nnlu2da93mp48/DATA_08.7z?dl=0).  
...
First we will use **gwr.sel()** function to find a bandwidth for a given geographically weighted regression by optimizing a selected function. For cross-validation, this scores the root mean square prediction error for the geographically weighted regressions, choosing the bandwidth minimizing this quantity
